...
class DataCenter_ITModel():
    ...
    def compute_datacenter_IT_load_outlet_temp(self,ITE_load_pct_list, CRAC_setpoint):
        ...
        for rack, rack_supply_approach_temp, ITE_load_pct \
            ...
            # Modify the delta check to make it less restrictive or adjust calculation parameters
            temp_delta = outlet_temp - rack_inlet_temp
            if temp_delta < 1.0:  # Changed from 2.0 to 1.0
                # Adjust the calculation to ensure a minimum delta of 1.0°C
                adjusted_delta = max(1.0, temp_delta)
                outlet_temp = rack_inlet_temp + adjusted_delta
                print(f'Adjusted outlet temperature to ensure minimum delta of 1.0°C. Original delta: {temp_delta:.3f}, New outlet temp: {outlet_temp:.3f}')
            elif temp_delta < 2.0:
                # For deltas between 1.0 and 2.0, just log a warning but don't trigger an exception
                print(f'Warning: Small temperature delta detected: {temp_delta:.3f}°C. This might indicate unusual conditions.')
